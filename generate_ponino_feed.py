...
SOURCE_URL = "https://www.doctorponino.com/products.json?limit=250"
OUTPUT = Path("doctor-ponino-meta-product-feed.xml")
BASE_URL = "https://www.doctorponino.com"
CURRENCY = "EGP"
...
request = urllib.request.Request(SOURCE_URL, headers={"User-Agent": "DoctorPoninoMetaFeed/1.0"})
with urllib.request.urlopen(request, timeout=60) as response:
    products = json.load(response).get("products", [])

rss = ET.Element("rss", {"version": "2.0"})
channel = ET.SubElement(rss, "channel")
add(channel, "title", "Doctor Ponino – Meta Product Feed")
add(channel, "link", BASE_URL)
add(channel, "description", "Doctor Ponino product catalog for Meta/Facebook and Instagram")
...
for product in products:
    ...
    brand = (product.get("vendor") or "Doctor Ponino").strip()
